...
工程设计还参考了 STORM、PaperQA2、AcaWriter、Papers-to-Posts 和 OpenScholar：证据按章节相关性选择，提纲显式保存判断与修辞动作，起草后通过确定性事实门禁和独立六维量表定向修订。证据到功能的完整映射见 [研究驱动的设计与成功门禁](docs/research-informed-design.md)。
...
来源被拆成带原始文档、位置、摘要和检索关键词的 Evidence Card。规划器按章节问题选择证据并拒绝未知 ID；生成正文使用 `[E:证据ID]` 临时绑定。审计器检查不存在的 ID、无证据数字、篇幅与分节完成度、未完成占位符、来源覆盖和案例文本重合，并给出 `one_shot_success` 严格门禁。当前版本导出的“来源台账”不是完整的 GB/T 7714 书目格式；学校模板和正式参考文献仍需作者核对。
...
- [研究驱动的设计与成功门禁](docs/research-informed-design.md)
- [一次生成成功率评测协议](docs/evaluation-protocol.md)

...
- 根因：CLI 必填参数测试依赖 Typer/Rich 渲染后的错误文本；GitHub Actions 的 ANSI 彩色输出和终端宽度使字符串断言不稳定，核心命令行为并未失效。
- 修复：直接检查 Typer/Click 命令对象中的 `provider` 与 `model` 必填契约，并保留缺参时退出码为 2 的行为验证。
- 验收：Python 3.11 与 3.13 全新环境在 `CI=true` 下均通过 37 项 pytest，覆盖率 83%；Ruff check、Ruff format check、严格 mypy 与 wheel 构建通过。
- 发布原则：保留已经公开的 `v0.3.0` 历史标签，不强制改写；以 `v0.3.1` 补丁标签发布修复。
